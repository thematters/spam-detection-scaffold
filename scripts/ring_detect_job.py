...
環境變數：
  ...
  AUTO_FREEZE_NEW_RATIO_HI  鑰2：新帳號比例門檻（預設 0.8）
  AUTO_FREEZE_BOT_RATIO_HI  舊設定，相容保留；亂碼帳號名不再單獨取得凍結資格
  AUTO_FREEZE_OLD_EXEMPT    舊設定，相容保留；新帳號比例門檻已涵蓋此護欄
  SHADOW_REPORT_OUT         精修後 v2 影子報告輸出路徑；只寫檔，不呼叫任何處置
...
def build_candidate(row: dict, items: list, count_col: str = "n_articles") -> dict:
    ...
    fps = [
        ring_signals.normalized_fingerprint(
            it.get("fingerprint_content", it.get("content", ""))
        )
        for it in items
    ]
    fingerprint = (
...
def _merge_by_fingerprint(cands: list) -> list:
    """把同指紋或共享強實體的候選 ring 合併成一筆（純函式，可測）：
    union 成員、重算 nAuthors、加總貼文數、合併訊號（ring size 取 max、codes/brands 取聯集）。

    強實體限於 invite/contact/domain；品牌詞太寬，不拿來跨指紋合併。
    """
    if not cands:
        return []
    ...
    for i, c in enumerate(cands):
        keys = {f"fp:{c['fingerprint']}", *strong_entity_keys(c.get("signals") or {})}
        for key in keys:
            if key in by_key:
                union(by_key[key], i)
            else:
    ...
    for group in groups.values():
        ...
        merged_sig = {
            ...
            "sampleTexts": _merge_sample_texts(sigs),
            "contentModelMax": None,
        }
        ...
        verified_members = {
            ...
            for c in group
            for member in (c.get("_verifiedMembers") or [])
            if member.get("id") is not None
        }
        if verified_members:
            merged["_verifiedMembers"] = [
                verified_members[uid] for uid in sorted(verified_members)
            ]
...
def strong_entity_keys(signals: dict) -> set:
    """Return conservative cross-template merge keys from app-layer signals."""
    keys = set()
    top = signals.get("topEntity")
...
def merged_fingerprint(group: list) -> str:
    """Keep original fingerprint for exact merges; use stable entity id for cross-template merges."""
    fps = {c["fingerprint"] for c in group}
    if len(fps) == 1:
        return next(iter(fps))
    keys = sorted({key for c in group for key in strong_entity_keys(c.get("signals") or {})})
    if keys:
        return "entity-" + hashlib.md5("|".join(keys).encode()).hexdigest()[:8]
    return sorted(fps)[0]


def _merge_sample_texts(signals_list: list, limit: int = 5) -> list:
    samples = []
    seen = set()
    for signals in signals_list:
        for text in signals.get("sampleTexts") or []:
            key = str(text).casefold()
            if key in seen:
                continue
            seen.add(key)
            samples.append(text)
...
def auto_freeze_eligible(cand: dict, *, high_authors: int = 3, new_ratio_hi: float = 0.8,
    ...
    """F3b（SPEC_RING_V2）自動凍結決策（純函式，可測）。

      雙鑰：跨帳號 ≥ high_authors（F1b 預設 3）＋新帳號比 ≥ new_ratio_hi
      帳號名亂碼只作人工佐證，不得單獨取得自動凍結資格。
    ...
    if ratio is None:
        return False
    # 兩個舊參數仍接受，避免既有 CodeBuild 設定在部署時失敗；決策已不使用它們。
    _ = bot_ratio_hi, old_exempt_ratio
    return ratio >= new_ratio_hi


def verified_member_ids(cand: dict) -> list[str]:
    """回傳候選成員與本輪內容驗證成員的交集，供影子與正式凍結共用。"""
    candidate_ids = {str(uid) for uid in (cand.get("memberUserIds") or [])}
    verified_ids = {str(uid) for uid in (cand.get("_verifiedMemberIds") or [])}
    return sorted(candidate_ids & verified_ids)


def shadow_decision(cand: dict, cfg: dict) -> dict:
    """產出精修後的單筆唯讀決策，口徑與 auto_freeze 完全共用。"""
    verified = verified_member_ids(cand)
    policy_eligible = auto_freeze_eligible(cand, **cfg)
    if not policy_eligible:
        action = "REVIEW_POLICY"
        reason = "requires_spread_and_high_new_account_ratio"
    elif cand.get("_truncated"):
        action = "REVIEW_TRUNCATED"
        reason = "candidate_posts_exceeded_verification_limit"
    elif len(verified) < cfg.get("high_authors", 3):
        action = "REVIEW_UNVERIFIED"
        reason = "verified_member_intersection_below_threshold"
    else:
        action = "AUTO_FREEZE_ELIGIBLE"
        reason = "spread_new_account_ratio_and_verified_members_passed"
    signals = cand.get("signals") or {}
    members_by_id = {
        str(member.get("id")): member
        for member in (cand.get("_verifiedMembers") or [])
        if member.get("id") is not None
    }
    return {
        ...
        "action": action,
        "reason": reason,
        "nPosts": int(cand.get("nArticles") or 0),
        "nAuthors": int(cand.get("nAuthors") or 0),
        "newAccountRatio": cand.get("newAccountRatio"),
        "botUsernameRatio": signals.get("botUsernameRatio"),
        "verifiedMemberCount": len(verified),
        "verifiedMembers": [
            members_by_id.get(uid, {"id": uid, "userName": None}) for uid in verified
        ],
        "truncated": bool(cand.get("_truncated")),
        "signals": {
            "nearDupRingSize": signals.get("nearDupRingSize"),
            "entityRingSize": signals.get("entityRingSize"),
            "topEntity": signals.get("topEntity"),
            "sampleCodes": signals.get("sampleCodes") or [],
            "sampleBrands": signals.get("sampleBrands") or [],
            "sampleTexts": signals.get("sampleTexts") or [],
        },
    }


def build_shadow_report(candidates: list, *, content_type: str, cfg: dict) -> dict:
    """產出純圖過濾、實體合併與驗證成員交集完成後的正式 v2 影子報告。"""
    decisions = [shadow_decision(cand, cfg) for cand in candidates]
    eligible = [d for d in decisions if d["action"] == "AUTO_FREEZE_ELIGIBLE"]
    eligible_accounts = {
        member["id"] for d in eligible for member in d["verifiedMembers"]
    }
    return {
        "schemaVersion": 1,
        "mode": "SHADOW_V2_POST_REFINEMENT_READ_ONLY",
        "generatedAt": dt.datetime.now(dt.timezone.utc).isoformat(),
        "sourceVersion": os.environ.get("CODEBUILD_RESOLVED_SOURCE_VERSION"),
        "contentType": content_type,
        "thresholds": {
            "highAuthors": cfg.get("high_authors", 3),
            "newAccountRatio": cfg.get("new_ratio_hi", 0.8),
        },
        "summary": {
            "candidateRings": len(decisions),
            "autoFreezeEligibleRings": len(eligible),
            "autoFreezeEligibleAccounts": len(eligible_accounts),
            "reviewRings": len(decisions) - len(eligible),
        },
        "decisions": decisions,
    }


def write_shadow_report(path: str, candidates: list, *, content_type: str, cfg: dict) -> None:
    report = build_shadow_report(candidates, content_type=content_type, cfg=cfg)
    Path(path).write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n")
    print(f"v2 shadow report: {path}")
