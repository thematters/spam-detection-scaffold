...
5. **security audit 跑過、無 CRITICAL/HIGH** → 開 AUTO_FREEZE。
   2026-07-05 差異審查結果：F1【HIGH】server 成員表為歷次 upsert 聯集，freezeSpamRing
   凍全部成員，job 端過濾作用不到 → 修法＝`FreezeSpamRingInput.memberUserIds` 交集凍結
   （server PR）＋ job 端只送「本輪 DB 驗證成員」、截斷/驗證數不足降級人工；
   F2【MED】merge ratio 改 nAuthors 加權（不取 max）；F3【MED】state 桶寫入策略須
   runner-only。**F1 server PR 部署 = AUTO_FREEZE 開啟的新增前置。**
6. AUTO_FREEZE 開啟首週：日報加註自動凍結清單，人工抽查；異常即關 env。

## 10a. 增量偵測（2026-07-04 增補，同輪實作）

使用者裁示：**全量掃不加頻**（30 天窗整掃 ×6 形狀不對），高頻靠**增量**——只掃新帳號/新文章。

- `scripts/ring_incremental_job.py`（JOB=`ring-incremental` / `moment-ring-incremental`）：
  S3 `ring-state/{content_type}/` 維護 30 天滾動貼文指紋狀態（app 端 normalized_fingerprint，
  純圖/空內容貼文**不進狀態**＝F3a 每貼文版）；每次只向 replica 抓水位線後新貼文
  （created_at 有索引，量 ∝ 發文速率），**只對被新貼文碰到的指紋群**重新分群；
  成員以最新 user.state 覆核後走與全量版完全相同的處置管線（upsert／AUTO_FREEZE）。
- 水位線在 upsert 成功後才推進（fingerprint 冪等，失敗重送安全）。
- 冷啟動只回看 24h，不重建歷史——**全量版降級為每日一次對帳**（漂移/漏網兜底），兩者並存。
- 成本（實測 general1.small $0.005/分、全量 ring 2.1 分＋moment 1.6 分）：
  全量每日 ≈ $0.6/月；增量每次估 <1 分鐘，每小時×2 jobs ≈ $7/月、每 15 分 ≈ $29/月。
  瓶頸不在 CodeBuild 費用而在容器啟動延遲（~1 分），高於每 15 分無意義。
