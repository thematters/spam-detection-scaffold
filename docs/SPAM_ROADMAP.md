...
**強化（2026-06-15 v2，`eval/ring_detect_poc.py`）**：(1) 近似比對 ring（char-4gram Jaccard≥0.5 union-find）比 exact 模板多抓「模板有變化」的群——老灯 exact 3→近似 26 帳號、披着 29、海外 26、高雄翻译社 11→14；(2) 帳號名亂碼分數（數字比例/長子音串/連續數字尾）當補充訊號——高雄翻译社 100% 亂碼帳號。資料源版 SQL `sql/detect_spam_rings.sql`（讀 replica 近期文章，DB 端按正規化模板 group by 粗篩、過濾新帳號，app 層再近似精修）。

**弱點/待解**：live173 文字變化太大（exact 24 族、近似 ring 仍只 4）→ 純文字相似度打不開，需「廣告同一實體」訊號（共用網域/聯絡方式/品牌詞）；繁簡字要正規化（實測「披著…」繁體零結果、簡體「披着…」28 帳號）。`detect_spam_rings.sql` 須 VPC 內執行（同 conformal 驗收/L1 那道牆）。
